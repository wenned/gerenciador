...
import { fecharMesa, fecthMesas} from '../Funcionalidades_adm/fecharMesa';
    ...
    useEffect(() => {
        async function capture() {
            try {
                ...
                setESTADO_GLOBAL(data);

            } catch (error) {
                console.error('Erro na API principal:', error);
                try {

                    const respostaFallback = await fecthMesas('mesas');
                    setESTADO_GLOBAL(respostaFallback);
                    
                } catch (fallbackError) {
                    console.error('Erro na API fallback:', fallbackError);
                }
            }
        }
